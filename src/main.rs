...
async fn main() {
    ...
    let stackexchange_results = StackExchange::get_questions_and_content(&search_text, term_width, enable_warnings);
    let github_gist_results = GithubGist::get_name_and_content(&search_text, enable_warnings);
    let geeksforgeeks_results = GeeksForGeeks::get_name_and_content(&search_text, term_width, enable_warnings);
    let duck_search_results = DuckSearch::get_name_and_content(&search_text, term_width, enable_warnings);
        ...
        match event_read {
            event::Event::Key(event::KeyEvent {
                ...
                //clearing the screen and printing our message
                    ...
                    Util::clear_terminal(&mut stdout, enable_warnings);
                    let content = match falion::get_key_map_with_vec(&stackoverflow_current, &mut stackoverflow_results, &mut stackoverflow_awaited, enable_warnings).await {
            ...
            event::Event::Key(event::KeyEvent {
                ...
                //clearing the screen and printing our message
                    ...
                    Util::disable_terminal_raw_mode(enable_warnings);
                    Util::clear_terminal(&mut stdout, enable_warnings);
                    let content = match falion::get_key_map_with_vec(&stackexchange_current, &mut stackexchange_results, &mut stackexchange_awaited, enable_warnings).await {
                        Some(content) => content,
                        None => {
                            vec![String::from("Warning! There was an error getting the content for the specified key.\nPress [Ctrl + q] to go back!")]
                        }
            ...
            event::Event::Key(event::KeyEvent {
                ...
                //clearing the screen and printing our message
                    ...
                    Util::disable_terminal_raw_mode(enable_warnings);
                    Util::clear_terminal(&mut stdout, enable_warnings);
                    let content = match falion::get_key_map_with_vec(&github_gist_current, &mut github_gist_results, &mut github_gist_awaited, enable_warnings).await {
                        Some(content) => content,
                        None => {
                            vec![String::from(" Warning! There was an error getting the content for the specified key.\nPress [Ctrl + q] to go back!")]
                        }
